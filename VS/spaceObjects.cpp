// This version is for Visual Studio for Windows - the header is different than mac OS - 
// the code is very much similar except with a few changes, the images may differ a little as this version is edited
// Project 3
// by Sumaiya Tabassum
...
void planetRollingOnRoad(void)
{
    ...
    for (int i = 0; i<2; i++)
    {
        ...
        glTranslated(0, -3, 0);
        glutSolidSphere(0.2, 50, 25); // Define object and size
        glPopMatrix();
    }
}
...
void object(void) // The image processed is better, and the road is completely flat - image does not show on windows but does on mac - used Mac to build

{
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, front_amb_diff_roadInSpace);
    glMaterialfv(GL_BACK, GL_AMBIENT_AND_DIFFUSE, front_amb_diff_roadInSpace);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, spe);
    glPushMatrix();
    glTranslated(0.1, 4.8, 0);
    glRotated(90, 1, 0, 0);
    GLUquadricObj* quadObj;
    quadObj = gluNewQuadric();
    gluQuadricDrawStyle(quadObj, GLU_LINE);
    gluCylinder(quadObj, 0.1, 0.4, 2.5, 80, 20);
    glPopMatrix();
    glPopMatrix();
}

void cannon(void) // Cannon in space - image was not showing on windows but shows on mac - used mac to build
...
void randomObject() { // Random object in space
    GLfloat front_amb_diff[] = { 0.7, 0.7, 0.7, 1.0 };
    GLfloat back_amb_diff[] = { 0.3, 0.3, 0.2, 1.0 };
    GLfloat spe[] = { 0.20, 0.15, 0.25, 1.0 };
    GLfloat amb[] = { 0.43, 0.11, 1, 1.0 };
    glPushMatrix();
...
void keyBoardFunctions() {
    ...
    cout << "Up Arrow - Speed up movement of center planets and move forward." << endl;
    cout << "Down Arrow - Slow down movement of center planets and eventually move backwards. " << endl;
    cout << "Space - Pause the program" << endl;
